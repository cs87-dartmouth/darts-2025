...
struct Photon
{
    ...
    uint16_t axis() const { return flags & SPLITTING_PLANE; }
    ...
    */
    ...
    Color3f power() const { return Color3f(rgbe[0], rgbe[1], rgbe[2]) * m_exp_table[rgbe[3]]; }
...
struct SearchResult
{
    ...
    bool operator<(const SearchResult &p2) const { return dist2 < p2.dist2; }
...
struct KNNSearch : public SearchBase
{
    ...
    KNNSearch(const Vec3f &q, float md2, unsigned max_count) : SearchBase(q, md2) { results.reserve(max_count); }
