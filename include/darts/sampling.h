...
inline float sample_circle_pdf() { return 0.5f * INV_PI; }
...
inline float sample_sphere_pdf() { return INV_FOURPI; }
...
inline float sample_4sphere_pdf() { return 0.5f * INV_PI * INV_PI; }
...
inline int sample_index(int size, float rv) { return clamp((int)(rv * size), 0, size - 1); }
...
struct Distribution1D
{
    ...
    */
    ...
    float discrete_PDF(int index) const { return func[index] / (func_int * count()); }
...
public:
    ...
    Distribution2D(const float *func, int nu, int nv)
    {
        ...
        for (int v = 0; v < nv; ++v) marginal_func.push_back(p_conditional[v]->func_int);
