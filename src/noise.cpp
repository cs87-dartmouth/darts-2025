...
inline Vec1f rand_dir(const Vec1i &i) { return Vec1f{2.f * hash_to_float(i) - 1.f}; }

inline Vec2f rand_dir(const Vec2i &i) { return sample_circle(hash_to_float(i)); }

inline Vec3f rand_dir(const Vec3i &i) { return sample_sphere(hash_to_float2(i)); }

inline Vec4f rand_dir(const Vec4i &i) { return sample_4sphere(hash_to_float4(i)); }
...
inline float perlin_noise_nd(const Vec<N, float> &p)
{
    ...
    for (auto grid : range(cell, cell + Vec<N, int>{2})) result += surflet(p - grid, rand_dir(grid));
...
float perlin_noise(const Vec1f &p) { return perlin_noise_nd(p); }

float perlin_noise(const Vec2f &p) { return perlin_noise_nd(p); }

float perlin_noise(const Vec3f &p) { return perlin_noise_nd(p); }

float perlin_noise(const Vec4f &p) { return perlin_noise_nd(p); }
