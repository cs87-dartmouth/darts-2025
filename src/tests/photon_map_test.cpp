...
Vec2f PhotonMapTest::sample_to_pixel(const Vec3f &pos) const { return pos.xy() * image_size; }

Vec3f PhotonMapTest::pixel_to_sample(const Vec2f &pixel) const { return Vec3f{pixel / image_size, 0.f}; }

Vec3f PhotonMapTest::generate_photon(const Vec2f &rv) const { return xform.point(Vec3f{sample_disk(rv), 0.f}); }
...
float PhotonMapTest::photon_density(const Vec3f &pos) const
{
    ...
    if (search_count > 0)
    {
        ...
        // their powers.
        for (auto photon : search.results) result += photon.photon->data.power();
...
void PhotonMapTest::run()
{
    ...
    Array2d<float> pdf(image_size.x, image_size.y);
    {
        ...
        for (int y = 0; y < pdf.height(); ++y, ++progress)
            ...
            {
                float accum = 0.f;
                for (uint32_t sx = 0; sx < super_samples; ++sx)
                    for (uint32_t sy = 0; sy < super_samples; ++sy)
                        accum += this->pdf(
