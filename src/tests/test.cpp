...
void run_tests(const json &j)
{
    ...
    if (j.contains("type") && j["type"] == "tests")
    {
        ...
        if (num_passed == count)
        {
            spdlog::info("Passed all {}/{} tests. Also examine any generated images.", num_passed, count);
            exit(EXIT_SUCCESS);
        }
        else
        {
            spdlog::error("Failed {}/{} tests. Also examine any generated images.", count - num_passed, count);
            exit(EXIT_FAILURE);
        }
    }
}
...
SampleTest::SampleTest(const json &j) : ScatterTest(j) { super_samples = j.value("super samples", 32); }
